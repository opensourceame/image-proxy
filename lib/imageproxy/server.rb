    ...
    def call(env)
      ...
      case options.command
        when "convert", "process", nil
          ...
          # check if we have a local cache of this file
          unless options.source.nil?

            cache_id     = Digest::MD5.hexdigest(env['QUERY_STRING'])
            cache_name   = cache_id + File.extname(options.source)

            if File.exists?(Cache.cache_dir + '/' + cache_name)
